...
regions = ["Control","Signal","LMVR","HMVR"]
#if o.bTagSyst: regions = ["Signal"]
names   = {"Control"   :"CR",
           "Signal"    :"hh",
           "Signal_in" :"hhi",
           "Signal_out":"hho",
...
def getAndStore(var,histName,suffix,jetSyst=False):
    ...
    for region in regions:
        if "bSF" in var and region != "Signal": continue
        #if "_cor" in var:
            # thisVar = var
            # if region == "LMVR": var = var.replace("_cor","_cor_Z")
            # if region == "HMVR": var = var.replace("_cor","_cor_H")
        h[region] = get(f, o.cut+"_"+tag+"Tag_"+region+"/"+var)
        h[region].SetName(histName.replace("_hh","_"+names[region])+suffix)
    ...
    if jetSyst:
        ...
        for region in regions:
            h_syst[region] = {}
            for syst in NPs:
                h_syst[region][syst[0]] = get(f_syst[syst[0]], o.cut+"_"+tag+"Tag_"+region+"/"+var)
                h_syst[region][syst[0]].SetName((histName+"_"+syst[0]).replace("_hh","_"+names[region])+suffix)

                if len(syst) == 2: #has up and down variation
                    h_syst[region][syst[1]] = get(f_syst[syst[1]], o.cut+"_"+tag+"Tag_"+region+"/"+var)
...
for suffix in ["","_l","_v"]:#,"_1"]:#,"_v_s"]:
    thisVar = var.replace("_f",suffix) if suffix else var
    getAndStore(thisVar,o.histName,suffix,o.jetSyst)
    if o.bTagSyst:
        for i in range(1,11)+range(47,49):
            getAndStore(thisVar+"_bSF"+str(i),o.histName+"_"+getBTagSFName(i),suffix)

    if o.trigSyst:
        for i in ["_tSF_up","_tSF_down"]:
            getAndStore(thisVar+i,o.histName+i,suffix)
